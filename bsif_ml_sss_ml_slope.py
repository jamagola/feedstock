...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 26 10:19:05 2019
...
def LR3(data, leaveout_lot):  
        ...
        input_train_data=training[:,[3,6,9]]
        input_test_data=testing[:,[3,6,9]]
            
        output_train_data = training[:,2] #Output: Slope         
        ...
        error=abs(testing[:,2]-predicted_slope)
        pError=100*error/testing[:,2]
        # Round to 2 decimal place
        temp = {'Slope':{leaveout_lot:testing[:,2]}, 'Predicted Slope':{leaveout_lot:predicted_slope.round(2)}, 'Absolute error':{leaveout_lot:error.round(2)}, '%Error':{leaveout_lot:pError.round(2)}} 
...
def KNNX(data, leaveout_lot): 
        ...
        # Round to 2 decimal place
        temp = {'Stress':{leaveout_lot:testing[0,4]},'Strain':{leaveout_lot:testing[0,5]},'Slope':{leaveout_lot:testing[0,6]}, 'Predicted Stress':{leaveout_lot:predicted_stress}, 'Absolute error':{leaveout_lot:error}, '%Error':{leaveout_lot:pError},'Predicted Strain':{leaveout_lot:predicted_strain}, 'Absolute error2':{leaveout_lot:error2}, '%Error2':{leaveout_lot:pError2}, 'Predicted Slope':{leaveout_lot:predicted_slope}, 'Absolute error3':{leaveout_lot:error3}, '%Error3':{leaveout_lot:pError3}} 
        info=pd.DataFrame.from_dict(temp)
        return info    
...
plt.xlabel('Material Lot')
plt.ylabel('%Error')
plt.title('Prediction error: LR-Slope-Scalar')
plt.legend('L')
plt.grid()
plt.show()